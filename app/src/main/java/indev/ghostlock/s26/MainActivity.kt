...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        b.deviceInfo.text = DeviceCompat.describe(ident)
        val (compatText, _) = DeviceCompat.compatText(ident)
        b.compatInfo.text = compatText
        ...
        // The output window scrolls on its own: keep drags inside it (don't
        // let the outer page steal them) and only auto-follow while the user
        // is already at the bottom — never yank them away from reading.
        b.logScroll.setOnTouchListener { v, event ->
    ...
    private suspend fun rootPipeline() {
        ...
        val ident = DeviceCompat.read()
        val (_, runnable) = DeviceCompat.compatText(ident)
        if (!runnable) {
    ...
    private suspend fun execAsRoot() {
        ...
        try {
            ...
            log("If KernelSU Manager shows this device rooted, open it and allowlist GhostLock S26, then retry.")
        }
    }
